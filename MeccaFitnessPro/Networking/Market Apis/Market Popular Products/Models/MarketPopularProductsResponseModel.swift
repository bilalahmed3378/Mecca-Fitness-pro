//
//  MarketPopularProductsResponseModel.swift
//  MeccaFitnessPro
//
//  Created by CodeCue on 28/06/2022.
//

import Foundation


struct MarketPopularProductsResponseModel : Codable {
    
    
    let status : String
    let code : Int
    let message : String
    let data : MarketPopularProductsDataModel?
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
        ...
        do{
            data = try container.decode(MarketPopularProductsDataModel?.self, forKey: .data) ?? nil
        }
        catch{
            data = nil
        }
...
struct MarketPopularProductsDataModel : Codable {
    ...
    let per_page : Int
    let total : Int
    let products : [MarketPopularProduct]
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
        ...
        do {
            products = try container.decode([MarketPopularProduct]?.self, forKey: .products) ?? []
        } catch  {
            products =  []
        }
...
struct MarketPopularProduct : Codable , Hashable{
