//
//  ViewAllEventsResponseModel.swift
//  MeccaFitnessPro
//
...
struct ViewAllEventsResponseModel : Codable {
    
    
    let status : String
    let code : Int
    let message : String
    let data : ViewAllEventsDataModel?
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
        ...
        do{
            data = try container.decode(ViewAllEventsDataModel?.self, forKey: .data) ?? nil
        }
        catch{
            data = nil
        }
...
struct ViewAllEventsDataModel : Codable {
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
        ...
        do {
            events = try container.decode([ViewAllEventModel]?.self, forKey: .events) ?? []
        } catch  {
            events =  []
        }
...
struct ViewAllEventModel : Codable , Hashable {
