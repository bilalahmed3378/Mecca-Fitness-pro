//
//  ViewAllShopPPResponseModel.swift
//  MeccaFitnessPro
//
...
struct ShopPopularProductsResponseModel : Codable {
    
    let status : String
    let code : Int
    let message : String
    let data : ShopPopularProductsDataModel?
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
           data = try container.decode(ShopPopularProductsDataModel?.self, forKey: .data) ?? nil
       } catch  {
           data = nil
       }
...
struct ShopPopularProductsDataModel : Codable {
    ...
    let per_page : Int
    let total : Int
    let products : [ShopPopularProductModel]
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
        ...
        do {
            products = try container.decode([ShopPopularProductModel]?.self, forKey: .products) ?? []
        } catch  {
            products =  []
        }
...
struct ShopPopularProductModel : Codable , Hashable {
