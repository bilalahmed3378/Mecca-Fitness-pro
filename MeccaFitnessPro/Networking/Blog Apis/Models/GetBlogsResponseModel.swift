...
struct GetAllBlogsResponseModel : Codable{
    
    
    let status : String
    let code : Int
    let message : String
    let data : GetAllBlogsDataModel?
    ...
    init(from decoder: Decoder) throws {
       ...
       let container = try decoder.container(keyedBy: CodingKeys.self)
        ...
        do {
            data = try container.decode(GetAllBlogsDataModel?.self, forKey: .data) ?? nil
        } catch  {
            data =  nil
        }
...
struct GetAllBlogsDataModel : Codable {
    ...
    let per_page : Int
    let total : Int
    let blogs : [GetAllBlogsBlogModel]
    ...
    init(from decoder: Decoder) throws {
       ...
       do {
        ...
        do {
            blogs = try container.decode([GetAllBlogsBlogModel]?.self, forKey: .blogs) ?? []
        } catch  {
            blogs =  []
        }
...
struct GetAllBlogsBlogModel : Codable , Hashable{
